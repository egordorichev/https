...
static void cleanup_request(LitState* state, LitUserdata* data, bool mark) {
	...
	if (request_data->total_length > 0) {
...
LIT_METHOD(secureNetworkRequest_contructor) {
...
#define FREE_HEADERS() \
	...
	sprintf(data->message, "%s %s%s HTTP/1.0\r\n", method_string, url_data.path, get && body != NULL ? body->chars : "");
	value_index = 0;
	...
	data->ssl_context = SSL_CTX_new(TLS_method());

	if (data->ssl_context == NULL) {
		fprintf(stderr, "%s\n", ERR_error_string(ERR_get_error(), NULL));
		free_parsed_url(state, &url_data);
		FREE_HEADERS()
		lit_runtime_error_exiting(vm, "Error creating SSL context");
	}

	data->connection = SSL_new(data->ssl_context);
	...
	if (data->connection == NULL) {
		...
		FREE_HEADERS()
		lit_runtime_error_exiting(vm, "Error creating SSL connection");
	}

	SSL_set_fd(data->connection, data->socket);
	int result = SSL_connect(data->connection);

	if (result != 1) {
		int error = SSL_get_error(data->connection, result);
		fprintf(stderr, "%s\n", ERR_error_string(ERR_get_error(), NULL));
		free_parsed_url(state, &url_data);
		SSL_CTX_free(data->ssl_context);
		SSL_free(data->connection);
		FREE_HEADERS()
		lit_runtime_error_exiting(vm, "SSL connection error %i", error);
	}
...
LIT_METHOD(secureNetworkRequest_write) {
	LitNetworkRequest* data = LIT_EXTRACT_DATA(LitNetworkRequest);
	int bytes = SSL_write(data->connection,data->message + data->bytes, data->total_length - data->bytes);
...
LIT_METHOD(secureNetworkRequest_read) {
	LitNetworkRequest* data = LIT_EXTRACT_DATA(LitNetworkRequest);
	LitState* state = vm->state;
